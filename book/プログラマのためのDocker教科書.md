...
コア機能がどのような仕組みで動いてるかについての記述
Docker は Linux カーネルの技術がベースになっている

### コンテナを区画する仕組み(namespace)

- Linux カーネルの namespace を用いる

  - Linux カーネルの namespace 機能は Linux のオブジェクトに名前をつけることで次の６つの独立した環境を構築できる

- PID namespace
  -  各プロセスに割り当てられた PID(プロセス ID)を用いる
- Newtwork namespace
  - ネットワークリソース(ネットワークデバイス、IP アドレス、 ポート番号など)を隔離された namespace で独立して持てる
- UID namespace
  - UID(ユーザー ID)や GID (グループ ID)を独立した namespace で持てる
- MOUNT namespace
  - マウント ... PC に接続した機器や記憶装置を OS に認識させて、利用可能な状態にすること
- IPC namespace
  - プロセス間通信(IPC) オブジェクトを namespace ごとに独立して持てる

### リソース管理の仕組み(cgroups)

- 物理マシン上のリソースを複数のコンテナで共用するため、あるプロセスが専有し過ぎて他のプロセスに影響を与えないようにリソースの割り当てを行う必要がある
- 上記を Linux カーネルの 「control groups(cgroups)」 によって実現している

### ネットワーク構成(仮想ブリッジ・仮想 NIC)

- コンテナが外部ネットワークと通信を行う仕組み

### Docker イメージのデータ管理の仕組み

- 「Copy on Write」方式を採用してイメージを保持する
  - 差分があったらメモリを確保してデータをコピーする
